...
class StockModel extends DbConfig
{
    ...
    public function get($storeditemid= null, $locationid= null, $report=false){
        ...
        $sql = 'SELECT s.*,i.price as lprice, i.name AS name, COALESCE(p.name, "Misc") AS supplier'.($report?', l.name AS location,i.price*s.stocklevel as stockvalue':'').' FROM stock_levels as s LEFT JOIN stored_items as i ON s.storeditemid=i.id LEFT JOIN stored_suppliers as p ON i.supplierid=p.id'.($report?' LEFT JOIN locations as l ON s.locationid=l.id':'');
        $placeholders = [];
