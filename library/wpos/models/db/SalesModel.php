...
class SalesModel extends TransactionsModel
{
    ...
    public function getGroupedTotals($stime, $etime, $status=null, $statparity= true, $grouptype = "device"){
        ...
        $placeholders = [":stime"=>$stime, ":etime"=>$etime];
        $sql = 'SELECT *, s.locationid as locationid, d.id as groupid, '.($grouptype=='device'?"CONCAT(d.name, ' (', l.name, ')')":'d.name').' as name, SUM(s.total) as stotal, COUNT(s.id) as snum FROM sales as s LEFT JOIN '.$joinsql.' WHERE (processdt>= :stime AND processdt<= :etime)';
